...
STATUS_POLL_INTERVAL = max(0.2, config.status_poll_interval)

# Topic existence probe interval
TOPIC_CHECK_INTERVAL = max(5.0, config.topic_check_interval)

# Last successful poll loop heartbeat (monotonic time).
_status_poll_heartbeat: float = 0.0


def get_status_poll_heartbeat_age() -> float | None:
    """Return seconds since status poll loop heartbeat, or None if never started."""
    if _status_poll_heartbeat <= 0.0:
        return None
    return max(0.0, time.monotonic() - _status_poll_heartbeat)
...
async def update_status_message(
    ...
    if interactive_window == window_id:
        ...
        if is_interactive_ui(pane_text, runtime=runtime):
    ...
    if should_check_new_ui and is_interactive_ui(pane_text, runtime=runtime):
        logger.debug(
...
async def status_poll_loop(bot: Bot) -> None:
    ...
    global _status_poll_heartbeat
    logger.info("Status polling started (interval: %ss)", STATUS_POLL_INTERVAL)
    last_topic_check = 0.0
    try:
        while True:
            _status_poll_heartbeat = time.monotonic()
            try:
