    ...
    async def get_pane_pid(self, window_id: str) -> int | None:
        ...
        def _sync_pane_pid() -> int | None:
            ...
            try:
                ...
                if not pane:
                    return None
                raw = getattr(pane, "pane_pid", None) or pane.get("pane_pid")
                if not raw:
                    return None
                return int(raw)
            except (ValueError, TypeError, AttributeError, Exception) as e:
                logger.debug("Failed to read pane_pid for %s: %s", window_id, e)
                return None

        return await asyncio.to_thread(_sync_pane_pid)
