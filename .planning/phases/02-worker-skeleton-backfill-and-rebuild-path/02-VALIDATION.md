...
| **Full suite command** | `uv run ruff check src/ tests/ && uv run ruff format --check src/ tests/ && uv run pyright src/codexbot/ && /tmp/codexbot-venv/bin/pytest -q` |
...
- **After every plan wave:** Run `uv run ruff check src/ tests/ && uv run ruff format --check src/ tests/ && uv run pyright src/codexbot/ && /tmp/codexbot-venv/bin/pytest -q`
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 02-W0-01 | 01 | 1 | INDX-02 | T-02-01 / T-02-05 | Worker startup is scheduled without blocking FastAPI/WebSocket/Telegram lifecycle and heavy imports stay out of request paths | unit + static | `uv run pytest tests/codexbot/test_search_worker.py tests/codexbot/test_web_server.py tests/codexbot/test_search_contracts.py -q` | `tests/codexbot/test_search_worker.py` | pending |
| 02-W0-02 | 01 | 1 | INDX-01 / INDX-08 | T-02-03 / T-02-04 | Worker/backfill state lives under `CODEXBOT_DIR/search` and incomplete generations are not active | unit | `uv run pytest tests/codexbot/test_search_state.py tests/codexbot/test_search_worker.py -q` | `tests/codexbot/test_search_worker.py` | pending |
| 02-W0-03 | 02 | 2 | CORP-01 / CORP-02 / INDX-03 | T-02-02 / T-02-06 | Backfill enumerates current tmux-backed sessions and parser-normalized text-bearing entries only | unit | `uv run pytest tests/codexbot/test_search_backfill.py tests/codexbot/test_transcript_parser.py -q` | `tests/codexbot/test_search_backfill.py` | pending |
| 02-W0-04 | 03 | 3 | INDX-01 / INDX-08 | T-02-03 / T-02-04 | Explicit rebuild creates a fresh generation and activates it atomically only after success | unit + CLI smoke | `uv run pytest tests/codexbot/test_search_worker.py tests/codexbot/test_search_state.py -q` | `tests/codexbot/test_search_worker.py` | pending |
| 02-W0-05 | 03 | 3 | INDX-02 / INDX-08 | T-02-03 / T-02-05 | Status reports `building` during backfill and `unavailable` with generation metadata after backfill succeeds before retrieval exists | API | `uv run pytest tests/codexbot/test_web_api.py::test_search_status_reports_building_backfill tests/codexbot/test_web_api.py::test_search_status_reports_generation_unavailable_after_backfill -q` | `tests/codexbot/test_web_api.py` | pending |
...
- [ ] `tests/codexbot/test_search_worker.py` - worker lifecycle, state placement, generation activation, rebuild, and interrupted generation assertions.
- [ ] `tests/codexbot/test_search_backfill.py` - open-session transcript source, ParsedEntry-to-document conversion, chunking, Codex/Claude fixture assertions.
- [ ] `tests/codexbot/test_web_api.py` additions - `building` and built-but-unavailable search status behavior.
- [ ] Existing `tests/codexbot/test_search_contracts.py` import-boundary test extended to keep worker/index dependencies out of `web/api.py` request paths.
